class RemoveTableOrganizations < ActiveRecord::Migration[5.0]
  def up
    ...
    execute(%q{UPDATE education_items AS e SET organization = o.name_en FROM organizations AS o WHERE e.organization_id = o.id})
    add_column :work_items, :organization, :string
    execute(%q{UPDATE work_items AS w SET organization = o.name_en FROM organizations AS o WHERE w.organization_id = o.id})
