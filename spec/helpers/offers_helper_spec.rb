...
RSpec.describe OffersHelper do
  ...
  describe '#readable_currency_range' do
    ...
    context 'when min and max present' do
      context 'when min == max' do
        it 'returns one value' do
          # 1000 PLN
          expect(helper.readable_currency_range((1000..1000), 'pln')).to eq(
            salary_string('mineqmax', 1000, 1000, 'pln')
          )
        end
      end
      ...
      context 'when min != max' do
        it 'returns a range' do
          # 1000-2000 PLN
          expect(helper.readable_currency_range((1000..2000), 'pln')).to eq(
            salary_string('minmax', 1000, 2000, 'pln')
          )
        end
      end
    end
    ...
    context 'when only min present' do
      it 'returns min+' do
        # 1000+ PLN
        expect(helper.readable_currency_range((1000..Float::INFINITY), 'pln')).to eq(
          salary_string('min', 1000, nil, 'pln')
        )
      end
    end
    ...
    context 'when only max present' do
      it 'returns up to max' do
        # up to 2000 PLN
        expect(helper.readable_currency_range((-Float::INFINITY..2000), 'pln')).to eq(
          salary_string('max', nil, 2000, 'pln')
        )
      end
    end
    ...
    context 'with -Infinity...Infinity' do
      it 'returns a placeholder string' do
        # none given
        expect(helper.readable_currency_range((-Float::INFINITY..Float::INFINITY), 'pln')).to eq(
          salary_string('none', nil, nil, 'pln')
        )
      end
    end
